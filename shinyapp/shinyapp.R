...
data <- data %>%
  ...
  mutate(
    region = case_when(
      zipcode %in% c(10004, 10005, 10006, 10007, 100388) ~ "Financial District",
      zipcode %in% c(10013, 10002) ~ "Chinatown",
      zipcode %in% c(10014, 10011) ~ "West Village",
      zipcode %in% c(10021, 10028, 10065, 10075, 10128) ~ "Upper East Side",
      zipcode %in% c(10023, 10024, 10025, 10069) ~ "Upper West Side",
      zipcode %in% c(10026, 10027, 10030, 10031, 10032, 10037, 10039) ~ "Harlem",
      zipcode %in% c(10032, 10033, 10040) ~ "Washington Heights",
      zipcode %in% c(10003, 10009) ~ "East Village",
      zipcode %in% c(10001, 10011) ~ "Chelsea",
      zipcode %in% c(10280, 10282) ~ "Battery Park City",
